            ...
            <div class={styles.contentGrid}>
              <div class={styles.description}>
                {memoizedDescription.value.map((paragraph, index) => (
                  <p key={index} class={styles.paragraph}>
                    {paragraph}
                  </p>
                ))}
              </div>
              {showSocialLinks &&
                memoizedSocialLinks.value &&
                memoizedSocialLinks.value.length > 0 && (
                  <div
                    class={styles.socialLinksContainer}
                    role="list"
                  ...
                  >
                    {memoizedSocialLinks.value.map((link) => (
                      ...
                      >
                        ...
                        <div>
                          <h3 class={styles.socialName}>{link.name}</h3>
                        </div>
                      </a>
                    ))}
                  </div>
